...
int CCPSendSubscribeMsg(CLIENT_S *c, CCP_SUBSCRIBE_S *req)
{
    ...
    msgLen = CCPSerializeSubscribeMsg(c, req);
    if (msgLen < 0)
    {
        WRITE_IOT_ERROR_LOG("serialize subscribe message failed!");
    ...
    if (NULL != c->onTimeout)
    {
        if (SUCCESS_RETURN != (rc = pushSubReqIntoUnackList(c, req, &node)))
        {
            WRITE_IOT_ERROR_LOG("push subscribe request into unack list failed!");
            aliyun_iot_mutex_unlock(&c->writeBufMutex);
            return rc;
        }
    }
...
int pushSubReqIntoUnackList(CLIENT_S *c, CCP_SUBSCRIBE_S *req, list_node_t **node)
{
    ...
    unack->req.msgType = SUBSCRIBE;
    unack->req.hasData = 1;
    CCP_SUBSCRIBE_S *subReq = (CCP_SUBSCRIBE_S *) aliyun_iot_memory_malloc(sizeof(CCP_SUBSCRIBE_S));
    if (NULL == subReq)
    {
    ...
    memset(subReq, 0, sizeof(CCP_SUBSCRIBE_S));

    *subReq = *req;
    unack->req.payload = subReq;
